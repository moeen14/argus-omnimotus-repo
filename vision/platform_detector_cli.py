...
ENGINE_PATH  = "highcam_platform.engine"
VIDEO_SOURCE = 2
IMG_SIZE     = 640
IOU_THRESH   = 0.75
...
CLASS_COLORS_TXT = {0: "\033[91m", 1: "\033[92m", 2: "\033[94m", 3: "\033[93m"}
RESET = "\033[0m"
...
def draw_detections(frame, detections, fps):
    ...
    counts = {cid: 0 for cid in CLASS_NAMES}
...
def open_camera(preferred_index=VIDEO_SOURCE):
...
def live_detect():
    ...
    print("=" * 60)
    print(" PLATFORM DETECTOR (highcam_platform TRT) - LIVE VIDEO")
    ...
    try:
        while True:
            ...
            class_maxes = raw[0, 4:, :].max(axis=1)
            debug_str = "  ".join(f"{CLASS_NAMES[i]}={class_maxes[i]:.2f}" for i in range(len(class_maxes)))
            print(f"[debug] raw max conf -> {debug_str}", end='\r')

            fps = 1.0 / max(1e-6, time.time() - t0)

            frame_with_detections = draw_detections(frame, detections, fps)
            cv2.imshow("PLATFORM DETECTOR (highcam_platform TRT)", frame_with_detections)
