...
class RepositoryTests(unittest.TestCase):
    def test_launcher_resolves_models_and_boards(self):
        run = module('run')
        script, models, env = run.prepare('autonomy', ROOT / 'config/robot.example.json')
    ...
    def test_launcher_rejects_path_escape(self):
        with self.assertRaises(ValueError):
            module('run').prepare('../outside.py', ROOT / 'config/robot.example.json')
    ...
    def test_all_imported_sources_present(self):
        records = json.loads((ROOT / 'docs/source-manifest.json').read_text())
        for record in records:
            self.assertTrue((ROOT / record['destination']).is_file(), record['destination'])
    ...
    def test_config_overrides_shell_device(self):
        with patch.dict('os.environ', {'ASABE_ESP1_PORT': '/dev/wrong'}):
            _, _, env = module('run').prepare('dashboard', ROOT / 'config/robot.example.json')
            self.assertEqual(env['ASABE_ESP1_PORT'], '/dev/esp32_01')

    def test_explicit_engine_override_is_preserved(self):
        with patch.dict('os.environ', {'ASABE_LANE_ENGINE': '/custom/rail.engine'}):
            _, _, env = module('run').prepare('autonomy', ROOT / 'config/robot.example.json')
            self.assertEqual(env['ASABE_LANE_ENGINE'], '/custom/rail.engine')

    def test_resource_links_reject_non_web_urls(self):
        with self.assertRaises(ValueError):
            module('update_links').render({'code': 'javascript:alert(1)'})

    def test_resource_links_render_public_urls(self):
        text = module('update_links').render({'code': 'https://github.com/example/argus'})
        self.assertIn('[Code](<https://github.com/example/argus>)', text)
        self.assertIn('Public URL has not been supplied yet.', text)
    ...
    def test_five_primary_applications(self):
        ...
        for name in ('dashboard', 'identify', 'navigate', 'field', 'lane'):
            target, _, _ = runner.prepare(name, ROOT / 'config/robot.example.json')
    ...
    def test_verification_dry_run_switches(self):
        ...
        for flag, expected in [('on', '1'), ('off', '0')]:
            result = subprocess.run([sys.executable, '-B', str(ROOT / 'tools/run.py'), '--config',
                                     str(ROOT / 'config/robot.example.json'), '--dry-run',
