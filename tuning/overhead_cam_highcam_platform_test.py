...
ENGINE_PATH = os.environ.get("ASABE_PLATFORM_ENGINE", os.path.join(os.path.dirname(__file__), "highcam_platform.engine"))
OVERHEAD_CAM_DEVICE = "/dev/v4l/by-id/usb-046d_0825_3D19ADE0-video-index0"
...
CONF_THRESH   = {0: 0.65, 1: 0.40, 2: 0.65, 3: 0.85}
COLOR_MAP     = {0: (0, 0, 255), 1: (0, 255, 0), 2: (255, 0, 0), 3: (0, 255, 255)}
PLATFORM_CLS  = 2
...
def draw(frame, detections, fps):
    ...
    best = max(
        ((conf, x1, y1, x2, y2) for cls_id, conf, x1, y1, x2, y2 in detections if cls_id == PLATFORM_CLS),
    ...
    if best:
        ...
        pcx = int((px1 + px2) / 2)
        err = (px1 + px2) / 2 - w / 2
        color = (0, 255, 0) if abs(err) < 50 else (0, 165, 255) if abs(err) < 100 else (0, 0, 255)
        for y in range(0, h, 20):
            cv2.line(frame, (pcx, y), (pcx, min(y + 10, h)), color, 2)
        cv2.putText(frame, f"Platform err: {err:+.1f}px", (10, h - 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
    else:
        cv2.putText(frame, "Platform: NOT DETECTED", (10, h - 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)


def open_overhead_cam():

    candidates = [OVERHEAD_CAM_DEVICE, OVERHEAD_CAM_INDEX] + [i for i in range(4) if i != OVERHEAD_CAM_INDEX]
    for src in candidates:
        cap = cv2.VideoCapture(src)
        if cap.isOpened():
            ok, _ = cap.read()
            if ok:
                print(f"[cam] opened overhead camera: {src}")
                return cap
        cap.release()
    return None
...
def main():
    ...
    cap = open_overhead_cam()
    if cap is None:
        print("[error] Could not open overhead camera.", file=sys.stderr)
        return 1
    cap.set(cv2.CAP_PROP_FRAME_WIDTH,  640)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)

    print("=" * 60)
    print(" OVERHEAD CAM  |  highcam_platform.engine")
    print(" classes: 0=ball  1=gcrop  2=platform  3=ycrop")
    ...
    try:
        while True:
            ...
            cv2.imshow("Overhead Cam — highcam_platform", frame)
