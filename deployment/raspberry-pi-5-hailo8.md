...
Obtain a **Hailo-8** HEF from the matching Hugging Face model release; Hailo-8L is a different compilation target. To compile an ONNX model, use the Hailo Dataflow Compiler on its supported build host:

```bash
python deployment/tools/hailo_compile.py --onnx /path/to/platform.onnx --calib /path/to/platform-dataset/images --nc 4 --arch hailo8 --out /path/to/hailo-output
```

The compiler needs its vendor SDK; installing HailoRT on the Pi alone does not supply it. The shared evaluator expects the exported YOLO output layout used by this project. HEFs with embedded NMS may require decoder changes.
