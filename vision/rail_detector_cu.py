...
SRC_PTS = np.array([[11, 343], [123, 83], [502, 84], [614, 344]], dtype=np.float32)
SRC_CM  = np.array([[0, 0], [0, 35], [35, 35], [35, 0]], dtype=np.float32)
PIXELS_PER_CM   = 3.0
...
class TRTSegDetectorCupy:
    def __init__(self, engine_path):
        ...
        for name in self.out_names:
            ...
            d_buf_cupy = cp.empty(shp, dtype=cp.float32)
            self.context.set_tensor_address(name, int(d_buf_cupy.data.ptr))
            role = "proto" if len(shp) == 4 else "det"
            self.out[role] = d_buf_cupy
    ...
    def infer(self):
        ...
        return self.out["det"], self.out["proto"]
...
def seg_postprocess_cupy(det_cp, proto_cp, r, pad, orig_shape):

    H, W = orig_shape[:2]


    p = det_cp[0].T
    conf = cp.max(p[:, 4:4 + NUM_CLASSES], axis=1)
    ...
    boxes_c = p_keep[:, :4].get()
    conf_c  = conf_keep.get()

    cx, cy, bw, bh = boxes_c[:, 0], boxes_c[:, 1], boxes_c[:, 2], boxes_c[:, 3]
    boxes_xyxy = np.stack([cx - bw/2, cy - bh/2, cx + bw/2, cy + bh/2], axis=1)

    idx = nms_cpu(boxes_xyxy, conf_c, IOU_THRESH)
    if not idx:
        return np.zeros((H, W), dtype=np.uint8), []
    ...
    coeffs = p_keep[idx, 4 + NUM_CLASSES:]
    protos = proto_cp[0]
    pc, ph, pw = protos.shape


    masks = cp.matmul(coeffs, protos.reshape(pc, ph * pw))
    masks = 1.0 / (1.0 + cp.exp(-cp.clip(masks, -88, 88)))
    masks = masks.reshape(-1, ph, pw)


    union_mask = cp.zeros((ph, pw), dtype=cp.bool_)
    ...
    for i in range(len(idx)):
        ...
        mx1, my1 = int(np.clip(bx1 * pw / IMG_SIZE, 0, pw)), int(np.clip(by1 * ph / IMG_SIZE, 0, ph))
        mx2, my2 = int(np.clip(bx2 * pw / IMG_SIZE, 0, pw)), int(np.clip(by2 * ph / IMG_SIZE, 0, ph))

        m_crop = cp.zeros_like(m)
        m_crop[my1:my2, mx1:mx2] = m[my1:my2, mx1:mx2]
        union_mask = union_mask | m_crop


    union_mask_cpu = union_mask.get().astype(np.uint8) * 255


    m640 = cv2.resize(union_mask_cpu, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_LINEAR)

    left, top = pad
    nh, nw = IMG_SIZE - 2*top, IMG_SIZE - 2*left
    m_unpad = m640[top:top+nh, left:left+nw] if (nh > 0 and nw > 0) else m640

    final_binary = cv2.resize(m_unpad, (W, H), interpolation=cv2.INTER_LINEAR)


    return final_binary, [final_binary > 0]
...
class RailNavProcessor:
    def __init__(self, engine_path, image_center_x, warmup_iters=5):
        ...
        print(f"Loading TRT Engine with CuPy: {self.engine_path}")
        self.detector = TRTSegDetectorCupy(self.engine_path)
    ...
    def process_frame(self, frame):
        ...
        det_cp, proto_cp = self.detector.infer()
        binary_mask, bool_masks = seg_postprocess_cupy(det_cp, proto_cp, r, pad, frame.shape)
        ...
        if have_rails:
            ...
            lateral_error = float(np.clip(
                (bottom_cx - self.image_center_x) / (FRAME_W / 2.0), -1.0, 1.0))
        ...
        disp_2 = cv2.cvtColor(binary_mask, cv2.COLOR_GRAY2BGR)
        ...
        if have_rails:
            ...
            cv2.polylines(disp_2, [project_line_safe(mc, bc, y_far, y_near, H_BEV2IMG)],
                          False, (255, 0, 255), 3)
        ...
        unified = np.hstack((rh(disp_1), rh(disp_2), rh(disp_3)))
...
def main():
    ...
    try:
        ...
        if processor: del processor.detector
        gc.collect()
        _CUDA_CONTEXT.pop()
