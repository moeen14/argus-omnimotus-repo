...
Upload the **contents of `Argus-Code`** as the code repository root. Keep `README.md`, `.github`, `.gitignore`, `firmware`, `applications`, `tuning`, `vision` and `deployment` at that level. Include hidden `.github`, `.gitignore` and `.gitattributes` files. The parent publication workspace and original export do not belong in this repository.
...
- Run `python tools/update_links.py --hardware-dir ../Argus-Hardware` to generate linked resource pages for both repositories. `--strict` additionally rejects missing URLs.
...
- Upload CAD and full circuit files to `Argus-Hardware` when supplied; finalize its purchasing BOM.
